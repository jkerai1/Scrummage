...
def Call_Plugin(**kwargs):
    ...
    elif kwargs["Plugin_Name"] == "Library Genesis Search":
        import plugins.Library_Genesis_Search as Library_Genesis_Search
        Thread_0 = threading.Thread(target=Starter, args=[kwargs["Task_ID"]])
        Thread_0.start()
        Thread_0.join()
        Thread_1 = threading.Thread(target=Library_Genesis_Search.Search, args=(kwargs["Query"], kwargs["Task_ID"], "board"), kwargs={"Limit": kwargs["Limit"], })
        Thread_1.start()
        Thread_1.join()
        Thread_2 = threading.Thread(target=Stopper, args=[kwargs["Task_ID"]])
        Thread_2.start()
